...
表\ref{table:ablation_2}展示了不同中级监督目标对预测误差的影响，第一行为去掉所有的中级监督目标只保留真值对网络最终输出的监督。它将作为基准网络来体现各种中级监督目标对网络的影响。第二行为使用真值作为所有阶段的中级监督目标，第三行为用高斯平滑构建中级监督目标。从表中可以看到，使用累积均值平滑（AAS）的本方法相比较上述的三种方案有明显优势，证明了AAS可以在多阶段网络中建立层次化的预测过程，有效降低了预测不确定性。
\subsection{对ST-DGCN图卷积模块的分析}
